    ...
    public async Task NativeRegionBeatsOptimizedTypedArrayPools()
    {
        ...
        using Process process = Process.Start(start)
            ?? throw new InvalidOperationException(
                "The Region regression process did not start.");
        ...
        catch (OperationCanceledException)
        {
            ...
            throw new TimeoutException(
                "The Region regression process exceeded ten seconds.");
        }

        string output = await outputTask;
        string error = await errorTask;
        RegionRegressionReport report =
            JsonSerializer.Deserialize<RegionRegressionReport>(
                output)
            ?? throw new InvalidDataException(
                "The Region regression report was empty.");
